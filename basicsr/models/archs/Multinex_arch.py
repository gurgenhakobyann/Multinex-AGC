...
try:
    from .agc_utils import compute_adaptive_gamma_luma, AdaptiveGammaHead
...
class IlluminationExtractor(nn.Module):
    ...
    def __init__(self,
                 ...
                 eps: float = 1e-6):
        super().__init__()
        # default: enable all six + agc
        default = dict(mean=True, rec709=True, vmax=True, lightness=True, ycgco=True, l2norm=True, agc=True)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use.get("agc", False):
            maps.append(compute_adaptive_gamma_luma(x, eps=self.eps))
...
class Multinex(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_adaptive_gamma_head: bool = False,
                 eps: float = 1e-6
                 ):
        super().__init__()
        ...
        # Adaptive Gamma Head (Strategy 3)
        self.use_adaptive_gamma_head = use_adaptive_gamma_head
        if self.use_adaptive_gamma_head:
            self.adaptive_gamma_head = AdaptiveGammaHead(C, eps=self.eps)
        else:
            self.adaptive_gamma_head = None
    ...
    def _fit_param_budget(self, target_params: int, in_ch: int, out_ch: int):
        ...
        for _ in range(10):
            ...
            if getattr(self, 'use_adaptive_gamma_head', False):
                self.adaptive_gamma_head = AdaptiveGammaHead(C, eps=self.eps)
